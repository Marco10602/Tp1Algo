...
\pred{esCamino}{\In distancias : \TLista{\TLista{\ent}}, \In desde : \ent, \In hasta : \ent, \In camino : \TLista{\ent}}{(|camino| > 1) \yLuego (camino[0] = desde \land camino[|camino|-1] = hasta) \land \paraTodo[unalinea]{i}{\ent}{(0 \leq i < |camino|) \implicaLuego (0 \leq camino[i]<|distancias|) \yLuego (hayCaminoDirecto(camino[i],camino[i+1],distancias))}}
...
\pred{matricesConsecutivas}{secuenciaDeMatrices : \TLista{\TLista{\TLista{\ent}}}}{\paraTodo[unalinea]{i}{\ent}{(0 \leq i < |secuenciaDeMatrices|-1) \implicaLuego (secuenciaDeMatrices[i+1] = multipDeMatrices(s[i], s[0]))}}

\pred{multipDeMatrices}{\In matriz1 : \TLista{\TLista{\ent}}, \In matriz2 \TLista{\TLista{\ent}}}{\paraTodo[unalinea]{x,y}{\ent}{(0 \leq x,y < |matriz1|) \implicaLuego (res[x][y] = fila X columna (x,y,matriz1,matriz2))}}

\aux{fila X columna}{\In x : \ent, \In y : \ent, \In matriz1 : \TLista{\TLista{\ent}}, \In matriz2 \TLista{\TLista{\ent}}}{\ent}{\sum\limits_{k=0}^{|matriz1|-1} (matriz1[x][k] * matriz2[k][y])}
...
\begin{proc}{caminoMinimo}{\In distancias : \TLista{\TLista{\ent}}, \In desde : \ent, \In hasta: \ent}{\TLista{\ent}}
	...
	\asegura{\paraTodo[unalinea]{s}{\TLista{\ent}}{(esCamino(distancias, desde, hasta, s)) \implicaLuego (distancia(distancias, s) \geq \\
	 distancia(distancias, res) )}}
\end{proc}

\aux{distancia}{\In distancias : \TLista{\TLista{\ent}},  \In camino:\TLista{\ent}}{\ent}{\sum\limits_{k=0}^{|s|-2} distancias[camino[k]][camino[k+1]]}

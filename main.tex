...
\begin{proc}{cantidadCaminosNSaltos}{\Inout conexión : \TLista{\TLista{\ent}}, \In n : \ent}{\TLista{\TLista{\ent}}}
	...
	\asegura{\existe[unalinea]{s}{\TLista{\TLista{\TLista{\ent}}}}{(|s|= n) \yLuego (\paraTodo[unalinea]{j}{\ent}{(0 \leq j < |s|) \implicaLuego (\paraTodo[unalinea]{i}{\ent}{(0 \leq i < |s[j]|) \implicaLuego (|s[j]| = |s[j][i]| = |conexion|)})}) \yLuego (s[0] = CONEXION_0) \yLuego (matricesConsecutivas(s)) \yLuego (conexion = s[|s| - 1])}}
\end{proc}
